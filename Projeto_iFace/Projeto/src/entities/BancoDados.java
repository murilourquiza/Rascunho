...
public class BancoDados {
	...
	private List <Mensagem> listMenUsu = new ArrayList<>();
	private List <Mensagem> listMenCom = new ArrayList<>();
	...
	public List<Mensagem> getListMenUsu() {
		return listMenUsu;
	}
	
	public List<Mensagem> getListMenCom() {
		return listMenCom;
	}
	...
	private void limparNaConta(String login) {
	...
	private void limparNaComunidade(String login) {
		for(Iterator<Comunidade> iterator = listCom.iterator(); iterator.hasNext(); ) {
	        ...
	        List <Mensagem> cancConta = new ArrayList<>();
	    	cancConta = val.getMensagens();
	    	for(Iterator<Mensagem> iterator2 = cancConta.iterator(); iterator2.hasNext();) {
	    		Mensagem val3 = iterator2.next();
	...
	private void limparNaMensagem(String login) {
		for(Iterator<Mensagem> iterator = listMenUsu.iterator(); iterator.hasNext(); ) {
	...
	private void limparNaSolAmi(String login) {
	...
	private void limparNaAmizade(String login) {
	...
	private void limparNoFeed(String login) {
	...
	public void cancelarConta(String login) {
		limparNaConta(login);
		limparNaComunidade(login);
	    limparNaMensagem(login);
	    limparNaSolAmi(login);
	    limparNaAmizade(login);
	    limparNoFeed(login);
	...
	public int enviarMenComunidade(String loginItinerario, String mensagem, String comunidade) {
		Mensagem aux = new Mensagem(loginItinerario, mensagem, comunidade);
		for(Comunidade val : listCom) {
			if(comunidade.contentEquals(val.getNome()) == true){
				listMenCom.add(new Mensagem(loginItinerario, mensagem, comunidade));
				val.addMensagem(aux);
				tamanhoListaCom = listCom.size();
				return 1;
			}
	...
	public void enviarMenUsuario(String loginItinerario, String mensagem, String loginDestinatario) {
		listMenUsu.add(new Mensagem(loginItinerario, mensagem, loginDestinatario));
		for(Conta val : list) {
	...
	private void addNovoAmigo(String login, String amigo) {
		for(Conta val : list) {
    		if(login.contentEquals(val.getLogin()) == true) {
    			val.addAmigos(amigo);
    			break;
	...
	public void addListAmi(String login, String amigo) {
		...
		addNovoAmigo(login, amigo);
		addNovoAmigo(amigo, login);
